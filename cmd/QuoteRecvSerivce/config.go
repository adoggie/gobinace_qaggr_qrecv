...
type Config struct {
	...
	RecvKlineEnable    bool `toml:"recv_kline_enable"`
	RecvAggTradeEnable bool `toml:"recv_aggtrade_enable"`

	RecvOrbIncReconnectTimeout int `toml:"recv_orbinc_reconnect_timeout"`
	RecvTradeReconnectTimeout  int `toml:"recv_trade_reconnect_timeout"`

	FanOutServers      []string `toml:"fanout_servers"`
	DataCompressed     bool     `toml:"data_compressed"`
	RecvSnapShotEnable bool     `toml:"recv_snapshot_enable"`

	SnapShotManagerConfig SnapShotManagerConfigVars `toml:"snapshot_manager"`
	DebugPrintTrade       bool                      `toml:"debug_print_trade"`
	MonConfig             core.ServiceMonConfigVars `toml:"qmon_manager"`

	DataEncodeType string `toml:"data_encode_type"`

	KlineClientManagerConfig    WsKLineMgrConfigVars    `toml:"kline_manager"`
	AggTradeClientManagerConfig WsAggTradeMgrConfigVars `toml:"aggtrade_manager"`
}
