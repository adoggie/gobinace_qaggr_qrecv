...
def on_message(ws, message):
    ...
    message = pm
    ts = datetime.datetime.now().timestamp()*1000
    logger.info(f"period:{message.period}, ts:{message.ts}, post_ts:{post_ts}, poster_id:{message.poster_id} , now:{ts} , ts dealy:{ ts - post_ts}")
...
专线
python ./ws_client_with_hdr.py run --user='4152cda6' --start=0 --ws_url="203.156.254.197:31501"
