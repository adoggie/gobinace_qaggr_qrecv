...
func normalize_config(cfg *Config) {
	...
	PERIOD = cfg.PeriodValue
	DETACH_TIME = cfg.PeriodDetachTime
	if cfg.UpdateTradingSymbolInterval == 0 {
		cfg.UpdateTradingSymbolInterval = 6 // hours
	}
	if cfg.TimeIterateInterval == 0 {
		cfg.TimeIterateInterval = 200 // micro seconds
	}
}
...
func main() {
	...
	logfile := path.Join(logdir, fmt.Sprintf("bn_das_%s_%s.log", config.ThunderId, config.Account))
	...
	{
		errlog := path.Join(logdir, fmt.Sprintf("bn_qas_%s_%s.err", config.ThunderId, config.Account))
	...
	////////////////////
	pidfile := fmt.Sprintf("/tmp/bn_qas_%s_%s.pid", config.ThunderId, config.Account)
